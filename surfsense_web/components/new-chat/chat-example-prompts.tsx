...
const CATEGORY_ICONS: Record<string, LucideIcon> = {
	destination: Binoculars,
	"traveler-insights": MessagesSquare,
	monitor: Radar,
};
...
const ExamplePromptButton = memo(function ExamplePromptButton({
	...
	return (
		...
		>
			<span className="min-w-0 text-pretty text-xs leading-5 sm:text-sm">{prompt}</span>
		</Button>
	);
});
...
export function ChatExamplePrompts({ onSelect }: ChatExamplePromptsProps) {
	...
	const categories = useMemo(() => getChatExampleCategories(locale), [locale]);
						...
						{categories.map((category) => {
							...
							return (
								<Button
									...
									className="h-8 rounded-lg bg-muted px-3 text-xs font-medium text-muted-foreground shadow-sm shadow-black/5 hover:bg-foreground/10 hover:text-foreground dark:shadow-black/10 sm:h-9 sm:rounded-xl sm:px-4 sm:text-sm"
								>
				...
				<div className="overflow-hidden rounded-lg border border-input/20 bg-muted shadow-sm shadow-black/5 dark:shadow-black/10 sm:rounded-xl">
					<div className="flex items-center justify-between gap-2 px-3 py-2 sm:gap-3 sm:px-4 sm:py-3">
						<div className="flex min-w-0 items-center gap-2 text-xs font-medium text-muted-foreground sm:text-sm">
							{(() => {
								const Icon = CATEGORY_ICONS[activeCategory.id] ?? WandSparkles;
								return <Icon aria-hidden="true" className="size-3.5 shrink-0 sm:size-4" />;
							})()}
						...
						<Button
							...
							aria-label={locale === "zh" ? "关闭研究模板" : "Close research templates"}
